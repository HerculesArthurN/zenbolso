import { Transaction, DashboardSummary, Badge } from '../types';
...
export const calculateBadges = (
  ...
  const hasInvestment = transactions.some(t => t.category.toLowerCase().includes('investimento') && t.type === 'expense'); 
  badges.push({
      id: 'badge-investor',
      key: 'investor',
      name: 'Investidor',
  ...
  const hasLazer = transactions.some(t => {
       ...
       const now = new Date();
       return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear() && t.category === 'Lazer';
  });
