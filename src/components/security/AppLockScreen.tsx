...
export function AppLockScreen() {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-zinc-900 flex flex-col items-center justify-between pb-12 pt-24 animate-in fade-in">
      <div className="flex flex-col items-center">
        <div className="w-16 h-16 bg-emerald-500/10 rounded-full flex items-center justify-center text-emerald-500 mb-6">
          <Lock size={32} />
        </div>
        <h1 className="text-2xl font-bold text-zinc-100 mb-2">Zenbolso Bloqueado</h1>
        <p className={`text-sm ${error ? 'text-rose-500 animate-bounce' : 'text-zinc-400'}`}>
          ...
          {[0, 1, 2, 3].map((index) => (
            <div 
              ...
              className={`w-4 h-4 rounded-full transition-colors ${
                pin.length > index ? 'bg-emerald-500' : 'bg-zinc-700'
              }`}
            />
          ))}
        </div>
      ...
      <div className="w-full max-w-[320px] grid grid-cols-3 gap-4 px-6">
        {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map((num) => (
          <button
            ...
            className="h-16 rounded-2xl bg-zinc-800 hover:bg-zinc-700 active:bg-zinc-600 text-2xl font-semibold text-zinc-100 transition-colors flex items-center justify-center"
          >
            {num}
          </button>
        ))}
        ...
        <button
          ...
          className="h-16 rounded-2xl text-zinc-400 hover:text-zinc-200 active:bg-zinc-800 transition-colors flex items-center justify-center"
        >
          <Delete size={28} />
        </button>
      </div>
      ...
      <button 
        ...
        className="text-zinc-500 hover:text-zinc-300 text-sm font-bold tracking-wide uppercase transition-colors mb-6"
      >
        Esqueci meu PIN
      </button>
      ...
      {showNuclearModal && (
        <div className="absolute inset-0 z-[110] bg-zinc-900/95 backdrop-blur-md flex items-center justify-center p-6 animate-in zoom-in-95 duration-200">
          <div className="bg-zinc-800 border border-rose-500/30 rounded-3xl p-8 max-w-[340px] text-center shadow-2xl flex flex-col gap-6">
            ...
            <div>
                <h2 className="text-xl font-bold text-zinc-100 mb-2 uppercase tracking-widest">Atenção Extrema</h2>
            ...
            <div className="flex flex-col gap-3">
              ...
              <button 
                ...
                className="w-full py-4 bg-zinc-700 hover:bg-zinc-600 active:scale-95 text-zinc-200 font-bold tracking-widest uppercase text-xs rounded-2xl transition-all"
              >
                Cancelar
              </button>
            </div>
