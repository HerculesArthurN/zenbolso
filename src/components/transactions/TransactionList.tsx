...
import { safeNumber, safeDivide } from '../../utils/numberUtils';
                    ...
                    <div key={date}>
                        ...
                        <div className="space-y-3">
                            {groupedTransactions[date].map((t) => {
                                ...
                                const visual = categoryMap.get(`${categoryName}-${t.type}`);
                                        ...
                                        <div className="flex items-center gap-4 overflow-hidden">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-center gap-2">
                                                    <p className="font-semibold text-slate-900 dark:text-white truncate">{categoryName}</p>
                                                </div>
                                        ...
                                        <div className="flex items-center gap-3 flex-shrink-0 pl-2">
                                            <div className="text-right">
                                                <span className={`font-bold tabular-nums tracking-tight block ${t.type === 'INCOME' ? 'text-income-text dark:text-income-dark' : 'text-slate-900 dark:text-white'
                                                    ...
                                                    {t.type === 'INCOME' ? '+' : '-'}
                                                    {currencyFormatter.format(safeNumber(t.amount))}
                                                </span>
                                                {t.type === 'EXPENSE' && hourlyRate > 0 && getTimeCost(safeNumber(t.amount)) && (
                                                    <span className="text-[10px] font-medium text-rose-500/80 flex justify-end items-center gap-1">
                                                        <Clock size={10} /> {getTimeCost(safeNumber(t.amount))}
                                                    </span>
                                                )}
                                            </div>
