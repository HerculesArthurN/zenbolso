...
export const GoalsWidget: React.FC = () => {
    ...
    const handleUpdateProgress = async (goal: Goal, amount: number) => {
        const newVal = Math.max(0, goal.currentAmount + amount);
        if (newVal > goal.targetAmount) return; // Prevent overfill for now
                                ...
                                <div className="absolute inset-0 p-3 flex flex-col justify-between">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-white/80 text-xs text-shadow">
                                            {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', notation: 'compact' }).format(goal.currentAmount)}
                                            /
                                            {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', notation: 'compact' }).format(goal.targetAmount)}
                                        </p>
                                    </div>
                                </div>
