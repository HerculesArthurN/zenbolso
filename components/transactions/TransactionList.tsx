...
export const TransactionList: React.FC<TransactionListProps> = ({ transactions, onDelete, onEdit, loading = false }) => {
  ...
  // Helper para buscar dados visuais da categoria
  const getCategoryVisuals = (catName: string, type: 'income' | 'expense') => {
            ...
            <div key={date}>
                ...
                <div className="space-y-3">
                    {groupedTransactions[date].map((t) => {
                        const { icon, color } = getCategoryVisuals(t.category, t.type);
                        ...
                        return (
                          ...
                          >
                              ...
                              <div className="text-right">
                                  <span className={`font-bold tabular-nums tracking-tight block ${
                                      t.type === 'income' ? 'text-income-text dark:text-income-dark' : 'text-slate-900 dark:text-white'
                                  }`}>
                                      {t.type === 'income' ? '+' : '-'} 
